...
const SUPPLY_POLL_SECONDS = Number(process.env.SUPPLY_POLL_SECONDS || 30);
const SUPPLY_ACTIVE_MINUTES = Number(process.env.SUPPLY_ACTIVE_MINUTES || 360);
const MAX_HOT_COLLECTIONS = Number(process.env.MAX_HOT_COLLECTIONS || 40);
const VOLUME_ALERT_COOLDOWN_SECONDS = Number(process.env.VOLUME_ALERT_COOLDOWN_SECONDS || 3600);
const SUPPLY_ALERT_COOLDOWN_SECONDS = Number(process.env.SUPPLY_ALERT_COOLDOWN_SECONDS || 1800);
...
let lastSupplyRunAt = 0;
let lastSupplyRunMs = 0;
let supplyRunInProgress = false;
let apiQueue = Promise.resolve();
let lastApiRequestAt = 0;

const sleep = ms => new Promise(r => setTimeout(r, ms));

// Serialize OpenSea REST calls and keep a small gap between requests. This avoids
// a burst of 40 supply requests hammering the API/Render free instance.
function queuedApi(fn) {
  const run = apiQueue.then(async () => {
    const gap = Math.max(0, 300 - (Date.now() - lastApiRequestAt));
...
async function osFetch(path) {
  return queuedApi(async () => {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      const key = apiKeys[Math.min(attempt, apiKeys.length - 1)];
      try {
        const r = await fetch(`https://api.opensea.io${path}`, {
          headers: { accept: "application/json", "x-api-key": key }
        });
        if (r.status === 429) {
          const retryRaw = Number(r.headers.get("retry-after"));
          const waitMs = Number.isFinite(retryRaw)
            ? Math.min(Math.max(retryRaw * 1000, 1000), 30000)
            : Math.min(1000 * 2 ** attempt, 15000);
          console.warn(`[OpenSea] 429 on ${path}, backing off ${Math.ceil(waitMs / 1000)}s`);
        ...
        lastErr = e;
        if (attempt < 3) await sleep(Math.min(500 * 2 ** attempt, 4000));
      }
    }
...
async function handleSale(event) {
  ...
  if (v && Date.now() - c.lastVolumeAlertAt >= VOLUME_ALERT_COOLDOWN_SECONDS * 1000) {
  ...
  setInterval(async () => {
    ...
    for (const c of collections.values()) {
      ...
      if (count >= SALES_PER_MINUTE) {
        // Sales alerts stay independent: at most one alert per wall-clock minute per collection.
...
async function supplyLoop() {
  const run = async () => {
    if (supplyRunInProgress) {
      console.warn("[SUPPLY] previous run still active; skipping overlap");
      return;
    }
    supplyRunInProgress = true;
    const started = Date.now();
    lastSupplyRunAt = started;
    try {
      const cutoff = started - SUPPLY_ACTIVE_MINUTES * 60_000;
      ...
      for (const c of hot) {
        try {
          ...
          if (old != null && fresh > old &&
              Date.now() - c.lastSupplyAlertAt >= SUPPLY_ALERT_COOLDOWN_SECONDS * 1000) {
      ...
      lastSupplyRunMs = Date.now() - started;
      console.log(`[SUPPLY] checked=${checked}/${hot.length} duration=${lastSupplyRunMs}ms`);
    } finally {
      supplyRunInProgress = false;
    }
  };

  // Run once shortly after startup, then on the configured interval.
  setTimeout(run, 5000);
  setInterval(run, SUPPLY_POLL_SECONDS * 1000);
}
...
app.get("/health", (_, res) => res.json({
  ...
  stream: "global-sales",
  lastStreamEventSecondsAgo: lastStreamEventAt ? Math.floor((Date.now() - lastStreamEventAt) / 1000) : null,
  lastStreamError: lastStreamError,
  lastStreamErrorSecondsAgo: lastStreamErrorAt ? Math.floor((Date.now() - lastStreamErrorAt) / 1000) : null,
  supply: {
    inProgress: supplyRunInProgress,
    lastRunSecondsAgo: lastSupplyRunAt ? Math.floor((Date.now() - lastSupplyRunAt) / 1000) : null,
    lastRunMs: lastSupplyRunMs,
    maxHotCollections: MAX_HOT_COLLECTIONS,
    intervalSeconds: SUPPLY_POLL_SECONDS
  }
}));
...
client.onItemSold("*", async (event) => {
  ...
  try {
    ...
    console.error("[SALE HANDLER]", err);
  }
});
...
console.log(`[RULE] Volume alert cooldown: ${VOLUME_ALERT_COOLDOWN_SECONDS}s per collection`);
console.log(`[RULE] Supply polling: every ${SUPPLY_POLL_SECONDS}s for up to ${MAX_HOT_COLLECTIONS} active collections (serialized)`);
console.log(`[RULE] Supply alert cooldown: ${SUPPLY_ALERT_COOLDOWN_SECONDS}s per collection`);
