...
const SALES_PER_MINUTE = Number(process.env.SALES_PER_MINUTE || 10);
const VOLUME_THRESHOLD_USD = Number(process.env.VOLUME_THRESHOLD_USD || 1000);
const MAX_VOLUME_WINDOW_MINUTES = Number(process.env.MAX_VOLUME_WINDOW_MINUTES || 500);
...
function extractSale(event) {
  ...
  const payment = sale?.payment_token ?? p?.payment_token ?? {};
  const usdPrice =
  ...
  const rawPrice =
    num(sale?.quantity) && num(sale?.unit_price) ? num(sale.unit_price) :
    num(sale?.price) ??
    num(sale?.sale_price) ??
    num(p?.price) ??
    num(p?.sale_price);

  const usd = usdPrice != null && rawPrice != null ? rawPrice * usdPrice : 0;

  const tokenId =
    getNested(event, [["payload","item","identifier"],["payload","item","token_id"],["payload","token_id"]]) ?? "";
  const tx =
    getNested(event, [["payload","transaction","hash"],["payload","transaction_hash"],["payload","tx_hash"]]) ?? "";

  return { ts, usd, tokenId: String(tokenId), tx: String(tx) };
}
...
function details(c) {
  ...
  const chain = col.chain || col.primary_chain || "unknown";
...
async function handleSale(event) {
  ...
  console.log(`[SALE] ${slug} | $${sale.usd.toFixed(2)} | sales_1m=${salesLastMinute(c)}`);
}

async function salesMinuteLoop() {
  // Every minute: exactly the requested fixed 1-minute sales rule.
  setInterval(async () => {
    const now = Date.now();
